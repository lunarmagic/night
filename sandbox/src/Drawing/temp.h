...
namespace night
{
    ...
    // two points
    static void lineFromPoints(vec2 P, vec2 Q, real& a,
        real& b, real& c)
    {
        a = Q.y - P.y;
        b = P.x - Q.x;
    ...
    static void perpendicularBisector(
    ...
    static vec2 lineLineIntersection(
        real a1, real b1,
        real c1, real a2,
        real b2, real c2)
    {
    ...
    static vec2 findCircumCenter(vec2 A[])
    {
        vec2 P, Q, R;
        P = A[0], Q = A[1], R = A[2];
    ...
    static vec2 findCentroid(vec2 A[])
    {
    ...
    // TODO: implement this.
    static vec2 calculate_orthocenter(const vec2& p1, const vec2& p2, const vec2& p3)
    {
        vec2 A[3] = {p1, p2, p3};
